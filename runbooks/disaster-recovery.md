...
**Document:** DR-1 | **Issue:** [PIX-4132](https://linear.app/pixelated/issue/PIX-4132)
...
Define Recovery Time Objective (RTO) and Recovery Point Objective (RPO) targets for every production service in the Pixelated Empathy platform, establish a service criticality classification framework, and document recovery procedures.
...
Services whose failure directly impacts patient safety, PHI integrity, or core therapeutic functionality.
...
Services whose degradation impacts user experience or data integrity but does not immediately endanger patient safety.
...
Services whose temporary loss is acceptable for up to 24 hours with minimal business impact.
...
3. Restart: `docker compose restart backend` or `kubectl rollout restart deployment/backend`
...
> **⚠️ PENDING** — The following stakeholders must review and approve these RTO/RPO targets:
...
Once signed off, this section will be updated and the RTO/RPO commitments are considered active.
