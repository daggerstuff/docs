...
- **Status:** **REMEDIATED (2026-07-30, PIX-4163).** Single pinning source was
  ...
  reference vllm). Bumped direct wheel pin to `vllm>=0.24.0 ; sys_platform ==
'linux'`; `uv lock --project` regenerated `uv.lock` and resolved vllm to
  **0.26.0** (PyPI), pulling **starlette 0.52.1 → 1.3.1** — the actual
  auth-bypass fix package. `uv tree` confirms both packages present in
  resolved graph. The same ≥0.24.0 floor also clears the 3 HIGH CVEs tracked
  in §3.1 (CVE-2026-54234, 55514, 55574).
- **TB4 verification (2026-07-30):** vllm is only imported by
  `ai/deployment/modal_app.py` (Modal A100 eval runner) and one
  `hackathon/smoke_tests/` smoke test — neither operates in the production
  web runtime path. TB4 threat-model §6 confirmed: vllm is not reachable from
...
- **Trivy re-run:** Pending — trivy CLI not installed in the remediation
  ...
  `art/_vllm_runtime/` directory after `uv sync` lands the wheels on a
  runtime platform (x86_64 sandbox cannot install `xgrammar 0.2.4`
  aarch64-only transitive wheel — same blocker exists at 0.20.x). Static
  graph verification (vllm 0.26.0 + starlette 1.3.1 resolved from PyPI) is
  the available evidence vector in this environment.
...
- ~~**starlette 0.52.1 → 1.3.1** — HTTP framework~~ ✅ **RESOLVED** (PIX-4161, 2026-07-30)
...
- `sharp` 0.34.5 → 0.35.0 (CVE GHSA-1124066) — _image processing; build-time risk_
- `react-router` 7.18.2 → 8.3.0 (CVE GHSA-1124282) — _major bump; migration planning_
- `brace-expansion` 1.1.16 → 5.0.8 (CVE GHSA-1124334) — _glob expansion; already in `resolutions` at 5.0.6 but pin not applied — needs investigation_
...
- ~~Tier 1 remediation PRs for: `pyjwt`, `python-multipart`, `starlette`~~ (starlette ✅ PIX-4161)
