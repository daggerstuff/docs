...
This document provides comprehensive documentation for all resources available
in the Journal Dataset Research MCP Server.
...
Resources are accessed via URIs and return structured data in JSON format.
...
**Description**: Current progress metrics for a research session, including
counts of sources identified, datasets evaluated, datasets acquired, and
integration plans created.

**Parameters**:

- `session_id` (string, required): Session ID (extracted from URI or provided in
  params)
...
**Description**: Historical progress snapshots for a research session, showing
progress over time.
...
**Update Frequency**: Updated with each progress snapshot (typically every 5
seconds during active operations)
...
**Description**: Complete session state including sources, evaluations,
...
**Update Frequency**: Updated when session state changes (sources added,
evaluations completed, etc.)
...
**Description**: Session metrics including activity logs, error logs, and
progress reports.
...
- Progress report: Updated periodically (every 5 seconds during active
  operations)
...
3. **Invalid URI format**: Use correct URI format:
   `research://{category}/{type}/{identifier}`
...
2. **Parameter Extraction**: Session ID can be extracted from URI or provided in
   params
3. **Update Frequency**: Resources update at different frequencies; check
   documentation
4. **Error Handling**: Always handle errors gracefully and check error codes
5. **Caching**: Resources are read-only; consider caching for frequently
   accessed data
6. **Authorization**: Ensure proper authentication and authorization before
   accessing resources
