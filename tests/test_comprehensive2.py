...
def test_fetch_extra():
    ...
    with patch('fetch_issues.requests.post') as m:
        def se(*args, **kwargs):
            ...
            r.json.return_value={"data": {"team": {"name": "TestTeam", "issues": {"edges": [{"node": {"id": "1", "identifier": "LIN-1", "title": "t", "state": {"type": "backlog"}, "priority": 1}}], "pageInfo": {"hasNextPage": False, "endCursor": None}}}}}
            return r
        m.side_effect=se
    ...
    flat = transform_to_flat({"id": "1", "identifier": "LIN-1", "title": "t", "state": {"type": "backlog"}, "priority": 1, "estimate": 2})
    assert flat["id"]=="1"
