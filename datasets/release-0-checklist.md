...
**Purpose**: One-page run sheet to produce the first versioned, S3-canonical
dataset release suitable for training.
...
- [ ] You are using S3-first behavior (training reads from S3, local is
      cache-only)
...
- [ ] At least one dataset exists under
...
- [ ] Write down the release prefix (example):
...
- [ ] Cross-split leakage checks pass for holdout-sensitive families (at
      minimum: edge/case and voice)
