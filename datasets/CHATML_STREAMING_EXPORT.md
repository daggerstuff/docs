...
Streaming exporter writes ChatML JSONL shards directly to S3 (no local files, no
full downloads).
...
- `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` (or use default AWS
  creds/profile)
- `CHATML_OUTPUT_PREFIX` (default: `releases/v2026-01-07/chatml`)
- `RELEASE_MANIFEST_KEY` (default:
...
- Shards:
  `s3://$AWS_S3_BUCKET/$CHATML_OUTPUT_PREFIX/<family>_chatml-shard-00001.jsonl`
- Summary:
  `s3://$AWS_S3_BUCKET/$CHATML_OUTPUT_PREFIX/release_0_export_summary.json`

## Behavior

- Streams JSONL via `iter_lines`, converts each record to ChatML, uploads via
  multipart (8MB parts, >=5MB safe).
