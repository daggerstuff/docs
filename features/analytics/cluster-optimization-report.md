...
- **CPU Utilization:** 0.5% actual usage vs 47.5% requested (2851m requested,
  31m actual)
...
**Analysis:** Traffic is very low, 3 replicas may be unnecessary for current
load
...
**Action:** Update Helm values to reduce resource requests **Savings:** ~1 CPU
core + ~1.9GB memory **Risk:** Low (current usage is very low)
...
**Action:** Increase memory requests to match actual usage **Savings:** Prevent
performance issues **Risk:** Low (fixes current under-provisioning)
...
**Action:** Scale down to 2 replicas, enable HPA for auto-scaling **Savings:**
Reduce by 33% (1 pod) **Risk:** Low (HPA will scale up if needed)
...
**Current:** 3 nodes × g4s.kube.medium (2 CPU, ~3.8GB RAM) **Analysis:** Total
usage is very low (~0.5% CPU, 36% memory)
...
**Action:** Add resource requests/limits to pods without them **Impact:** Better
scheduling, predictable resource usage **Risk:** Low
...
**Action:** Install VPA to automatically adjust resource requests **Impact:**
Continuous optimization based on actual usage **Risk:** Medium (requires careful
configuration)
...
**Action:** Set namespace-level resource quotas **Impact:** Prevent resource
waste, cost accountability **Risk:** Low

#### 3.2 Regular Cost Auditing

**Action:** Monthly review of resource usage and costs **Impact:** Continuous
