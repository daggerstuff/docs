...
description: Pixelated Empathy's procedure for identifying, tracking, remediating, and escalating control deficiencies. SOC2 CC4 compliance.
---
...
This procedure defines how Pixelated Empathy identifies, tracks, remediates,
and escalates control deficiencies. It satisfies SOC2 CC4 (Monitoring
Activities) requirements for deficiency identification, communication, and
remediation.
...
All deficiencies are tracked in the **Deficiency Log** (Linear project:
"Control Deficiencies" or spreadsheet if Linear not available).
...
- **Risk Accepted**: CSO has accepted the risk (with justification and review date).
...
2. **Remediation options**: What can fix it? (preventive, detective, corrective controls)
...
6. **Interim controls**: What compensating controls mitigate risk until full remediation?
...
4. **Status update**: Mark deficiency as "Verified" if effective, "Reopened" if not.
...
**Scope**: All key controls identified in SOC2 control mapping.

**Procedure**:

1. **Select sample**: For each control, select sample of operations (min 25 or 10% of population).
...
5. **Identify deficiencies**: Any failures are deficiencies (classify per Section 2).
...
**Audience**: Board (or leadership team).
**Content**:
...
_Document owner: Compliance Lead_
