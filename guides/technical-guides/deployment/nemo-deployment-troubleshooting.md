...
**Cause**: NGC (NVIDIA GPU Cloud) requires authentication to download resources.
Direct curl/wget downloads don't work without proper authentication.
...
4. Copy the API key (this is different from your NVIDIA_API_KEY for
   build.nvidia.com)
...
1. **Download from NGC Catalog:**
   - Visit:
     https://catalog.ngc.nvidia.com/orgs/nvidia/teams/nemo/resources/nemo-microservices-quickstart
...
- NGC API key (from catalog.ngc.nvidia.com) is different from NVIDIA_API_KEY
...
1. Check the logs:
   `ssh vivi@212.2.244.60 'cd ~/nemo-microservices && docker compose logs'`
